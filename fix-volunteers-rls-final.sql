-- Final fix for volunteers table RLS policies
...
-- 3. Disable RLS temporarily to test access
ALTER TABLE volunteers DISABLE ROW LEVEL SECURITY;

-- 4. Test access without RLS
SELECT COUNT(*) as volunteer_count_without_rls FROM volunteers;
...
-- 7. Create new policies that allow public access
...
CREATE POLICY "Enable update for all users" ON volunteers
